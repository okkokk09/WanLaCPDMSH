      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 no-print">
        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">โควตารวมทั้งหมด</span>
            <div className="p-2 bg-indigo-50 dark:bg-indigo-950/60 rounded-xl text-indigo-600 dark:text-indigo-400">
              <PieChart className="w-4 h-4" />
            </div>
          </div>
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-slate-900 dark:text-white">{totals.totalQuota}</span>
            <span className="text-xs text-slate-500 dark:text-slate-400">วัน</span>
          </div>
          <p className="mt-1 text-[11px] text-slate-400 dark:text-slate-500">จากบุคลากร {staffList.length} คน</p>
        </div>

        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">ใช้วันลาแล้วรวม</span>
            <div className="p-2 bg-amber-50 dark:bg-amber-950/60 rounded-xl text-amber-600 dark:text-amber-400">
              <ShieldAlert className="w-4 h-4" />
            </div>
          </div>
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-amber-600 dark:text-amber-400">{totals.totalUsed}</span>
            <span className="text-xs text-slate-500 dark:text-slate-400">วัน</span>
          </div>
          <p className="mt-1 text-[11px] text-slate-400 dark:text-slate-500">บันทึกการลาทั้งหมดในระบบ</p>
        </div>

        <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-2xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">วันลาคงเหลือรวม</span>
            <div className="p-2 bg-emerald-50 dark:bg-emerald-950/60 rounded-xl text-emerald-600 dark:text-emerald-400">
              <ShieldCheck className="w-4 h-4" />
            </div>
          </div>
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{totals.totalRemaining}</span>
            <span className="text-xs text-slate-500 dark:text-slate-400">วัน</span>
          </div>
          <p className="mt-1 text-[11px] text-emerald-700 dark:text-emerald-400">สามารถใช้ลาได้ตามสิทธิ์</p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 shadow-2xs flex flex-col sm:flex-row items-center justify-between gap-3 no-print">
        <div className="flex flex-1 items-center gap-3 w-full sm:w-auto">
          ...
          <div className="w-44">
            ...
            >
              <option value="all" className="bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200">ทุกกลุ่มงาน ({staffList.length})</option>
        ...
        <div className="text-xs text-slate-500 dark:text-slate-400">
          แสดง <b className="text-slate-800 dark:text-slate-200">{filteredSummaries.length}</b> คน
        </div>
      </div>
          ...
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="bg-slate-50/80 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 font-bold uppercase tracking-wider">
                ...
                <th className="py-3 px-3 text-center bg-rose-50/40 dark:bg-rose-950/30 text-rose-800 dark:text-rose-300 border-x border-slate-100 dark:border-slate-800">
                  ลาป่วย <br />
                  <span className="text-[10px] font-normal text-rose-600 dark:text-rose-400">ใช้ / โควตา (คงเหลือ)</span>
                </th>
                <th className="py-3 px-3 text-center bg-blue-50/40 dark:bg-blue-950/30 text-blue-800 dark:text-blue-300 border-r border-slate-100 dark:border-slate-800">
                  ลากิจ <br />
                  <span className="text-[10px] font-normal text-blue-600 dark:text-blue-400">ใช้ / โควตา (คงเหลือ)</span>
                </th>
                <th className="py-3 px-3 text-center bg-emerald-50/40 dark:bg-emerald-950/30 text-emerald-800 dark:text-emerald-300 border-r border-slate-100 dark:border-slate-800">
                  ลาพักผ่อน <br />
                  <span className="text-[10px] font-normal text-emerald-600 dark:text-emerald-400">ใช้ / โควตา (คงเหลือ)</span>
                </th>
                <th className="py-3 px-3 text-center bg-purple-50/40 dark:bg-purple-950/30 text-purple-800 dark:text-purple-300 border-r border-slate-100 dark:border-slate-800">
                  อื่นๆ <br />
                  <span className="text-[10px] font-normal text-purple-600 dark:text-purple-400">ใช้ / คงเหลือ</span>
                </th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filteredSummaries.map((s) => {
                ...
                return (
                  <tr key={s.staff.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                    {/* Staff info */}
                    <td className="py-3 px-4">
                      <div className="font-bold text-slate-900 dark:text-white">{s.staff.name}</div>
                    ...
                    {/* Sick Leave */}
                    <td className="py-3 px-3 text-center border-x border-slate-100 dark:border-slate-800">
                      <div className="font-semibold text-slate-800 dark:text-slate-200">
                        <span className="text-rose-600 dark:text-rose-400">{sickUsed}</span> / {sickQuota}
                      </div>
                    ...
                    <td className="py-3 px-3 text-center border-r border-slate-100 dark:border-slate-800">
                      <div className="font-semibold text-slate-800 dark:text-slate-200">
                        <span className="text-blue-600 dark:text-blue-400">{bizUsed}</span> / {bizQuota}
                      </div>
                      <div className="text-[11px] text-emerald-700 dark:text-emerald-400 font-medium">
                        เหลือ {bizRemain} วัน
                      </div>
                    ...
                    <td className="py-3 px-3 text-center border-r border-slate-100 dark:border-slate-800">
                      <div className="font-semibold text-slate-800 dark:text-slate-200">
                        <span className="text-emerald-600 dark:text-emerald-400">{vacUsed}</span> / {vacQuota}
                      </div>
                      <div className="text-[11px] text-emerald-700 dark:text-emerald-400 font-medium">
                        เหลือ {vacRemain} วัน
                      </div>
                    ...
                    {/* Other leaves */}
                    <td className="py-3 px-3 text-center border-r border-slate-100 dark:border-slate-800">
                      <div className="font-semibold text-slate-700 dark:text-slate-300">{otherUsed} วัน</div>
                    </td>

                    {/* Total Used and Remaining */}
                    <td className="py-3 px-4 text-center">
                      <div className="font-bold text-slate-900 dark:text-white text-sm">{s.totalUsed} วัน</div>
                      <div className="text-[11px] text-slate-400 dark:text-slate-500">จากโควตา {s.totalQuota} วัน</div>
                      <div className="text-[11px] text-emerald-700 dark:text-emerald-400 font-medium">คงเหลือรวม {s.totalRemaining} วัน</div>
                    </td>
                  </tr>
                );
              })}
